...
View(urbanicity_map)
write.csv(urbanicity_map,'urbanicity_map.cvs')

url <- 'https://raw.githubusercontent.com/plotly/datasets/master/geojson-counties-fips.json'
counties <- rjson::fromJSON(file=url)
url2<- 
df <- read.csv(url2, colClasses=c(county_name="character"))
g <- list(
  scope = 'usa',
...
fig2 <- fig2 %>% add_trace(
  type="choropleth",
  geojson=counties,
  locations=urbanicity_map$county_name,
  z=urbanicity_map$total_pop,
  colorscale="Viridis",
  zmin=0,
  zmax=2535942,
